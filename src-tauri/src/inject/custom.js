function onDocumentElementReady() {
  ...
  function injectStyles(styleString, cssFileName) {
    ...
    styles.forEach((styleString2) => {
      const styleElement = document.createElement("style");
      styleElement.textContent = styleString2;
  ...
  const homeIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>`;
  const gradeIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm-2 14l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z"/></svg>`;
  const notificationIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>`;
  ...
  async function getBottomInset() {
    return await invoke("plugin:safe-area-insets-css|get_bottom_inset", {
      payload: {}
    });
  }
  async function onKeyboardShown(handler) {
    return await addPluginListener("safe-area-insets-css", "keyboard_shown", handler);
  }
  async function onKeyboardHidden(handler) {
    return await addPluginListener("safe-area-insets-css", "keyboard_hidden", handler);
  }
  async function init() {
    const topInset = await getTopInset();
    const bottomInset = await getBottomInset();
    if (topInset) {
      document.documentElement.style.setProperty("--safe-area-inset-top", `${topInset == null ? void 0 : topInset.inset}px`);
    }
    if (bottomInset) {
      document.documentElement.style.setProperty("--safe-area-inset-bottom", `${bottomInset == null ? void 0 : bottomInset.inset}px`);
    }
    onKeyboardShown(() => {
  ...
  function initializeCustomSelects() {
    ...
    const updateSelectButtonLabel = (select, buttonLabel) => {
      ...
      buttonLabel.textContent = ((_a = selectedOption == null ? void 0 : selectedOption.textContent) == null ? void 0 : _a.trim().replace(/\(\d+-\d+学年第\d学期\)/g, "")) || "请选择";
    };
    ...
    const syncSelectOptions = (select, wrapper, buttonLabel, optionsPanel) => {
      ...
      Array.from(select.options).forEach((option, index) => {
        ...
        optionButton.textContent = ((_a = option.textContent) == null ? void 0 : _a.trim().replace(/\(\d+-\d+学年第\d学期\)/g, "")) || option.value || `选项 ${index + 1}`;
    ...
    const scanSelects = (root = document) => {
      var _a;
      const selects = root instanceof HTMLSelectElement ? [root] : (_a = root.querySelectorAll) == null ? void 0 : _a.call(root, "select");
  ...
  function removeCourseSerialNumbers() {
    ...
    if (isAlertsStreamPage) {
      let alertCleanupTimer2;
      const stripAlertSerials2 = () => {
        ...
        if (courseLinks.length !== 0 && alertCleanupTimer2) {
          clearInterval(alertCleanupTimer2);
        }
      };
      stripAlertSerials2();
      alertCleanupTimer2 = setInterval(() => {
        const courseLinks = document.querySelectorAll("#streamHeader_alerts a");
...
${downloadUrl}`);
  ...
  function initializeMobileCourseHeaderLayout() {
    ...
    const restoreNode = (record) => {
      if (!(record == null ? void 0 : record.node) || !record.parent) {
        return;
      }
      const referenceNode = record.nextSibling && record.nextSibling.parentNode === record.parent ? record.nextSibling : null;
    ...
    const moveNodesIntoHeader = () => {
      ...
      if (records.breadcrumbs.node.parentNode === headerHost && records.pageTitleHeader.node.parentNode === headerHost) {
        document.body.classList.add(bodyClassName);
        const pageTitleDiv2 = document.querySelector("#pageTitleDiv");
        if (pageTitleDiv2) {
          pageTitleDiv2.classList.add(hiddenClassName);
        }
        return true;
      }
    ...
    const restoreOriginalLayout = () => {
      ...
      if (state.records.breadcrumbs.node.parentNode === state.records.breadcrumbs.parent && state.records.pageTitleHeader.node.parentNode === state.records.pageTitleHeader.parent) {
        document.body.classList.remove(bodyClassName);
        const pageTitleDiv2 = document.querySelector("#pageTitleDiv");
        if (pageTitleDiv2) {
          pageTitleDiv2.classList.remove(hiddenClassName);
        }
        return;
      }
  ...
  async function autoLogin() {
    ...
    try {
      ...
      console.debug("[PKU Art] Skip autoLogin because cookies are unavailable in this document", error2);
      return;
    }
    console.debug(window.location.href, cookie);
    if (cookie.includes("course_login=true") && Date.now() - ((_a = cookie.match(/course_last_login=([^;]*)/)) == null ? void 0 : _a[1]) < 1e3 * 60 * 60 * 3) {
      return;
    }
    const store2 = await getStore();
  ...
  async function portalLogin() {
    ...
    try {
      ...
      console.debug("[PKU Art] Skip portalLogin because cookies are unavailable in this document", error2);
      return;
    }
    if (cookie.includes("portal_login=true") && Date.now() - ((_a = cookie.match(/portal_last_login=([^;]*)/)) == null ? void 0 : _a[1]) < 1e3 * 60 * 60 * 3) {
      return;
    }
...
<div class="pku-art-container">
  ...
  function isInjectedPdfViewerFrame() {
    var _a, _b;
    return window.location.protocol === "blob:" || ((_b = (_a = window.frameElement) == null ? void 0 : _a.classList) == null ? void 0 : _b.contains("pku-art-pdf-frame"));
  }
